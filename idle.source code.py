...
class IMG_Stegno:
    ...
    def main(self, root):
        root.title('ImageSteganography by NIMISHA ')
        ...
        encode = Button(frame,text="Encode",command= lambda :self.encode_frame2(frame), padx=14,bg = '#e3f4f1' )
        encode.config(font=('Helvetica',14), bg='#e8c1c7')
        encode.grid(row=2)
        decode = Button(frame, text="Decode",command=lambda :self.decode_frame2(frame), padx=14,bg = '#e3f4f1')
